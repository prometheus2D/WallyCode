...
internal sealed class SetupCommandHandler
{
    ...
    public Task<int> ExecuteAsync(SetupCommandOptions options, CancellationToken cancellationToken)
    {
        ...
        if (options.Cleanup || options.Install)
        {
        ...
        if (options.Install)
        {
            var installHandler = new InstallCommandHandler(_providerRegistry, _logger, _appDirectoryPath);
            installHandler.ExecuteAsync(
                new InstallCommandOptions { SourcePath = targetDirectory },
                cancellationToken).GetAwaiter().GetResult();
        }
        else
        {
